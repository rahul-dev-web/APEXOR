...
This command connects to Discord, inspects every guild visible to the bot, and
returns a machine-readable report. It intentionally performs no Discord
mutations: it never creates roles/channels, changes permissions, or runs
...
Exit codes:
    0 - every visible guild passes the required checks.
...
def inspect_guild(guild: discord.Guild, bot_user_id: int | None) -> GuildPreflight:
    ...
    bot_permissions = {
        name: bool(getattr(me.guild_permissions, name, False))
        for name in REQUIRED_BOT_PERMISSIONS
    } if me is not None else {name: False for name in REQUIRED_BOT_PERMISSIONS}
    ...
    for role in guild.roles:
        ...
        if not is_owner_role and not above_bot:
            findings.append(
                RoleFinding(
                    role_id=role.id,
                    role_name=role.name,
    ...
    if findings:
        failures.append(
            ...
            + "; ".join(
                f"{item.role_name} ({', '.join(item.protected_permissions)})"
    ...
    return GuildPreflight(
        ...
        manageable_protected_roles=tuple(findings),
