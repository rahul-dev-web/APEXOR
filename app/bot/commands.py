...
async def _authorized(interaction: discord.Interaction, capability: Capability) -> bool:
    ...
    if not allowed:
        await interaction.response.send_message(f"Access denied. Required APEXOR capability: `{capability.value}`.", ephemeral=True)
    return allowed


async def _protected_resource_allowed(interaction: discord.Interaction, *, resource_type: str, resource_id: int) -> bool:
    """Protected APEXOR resources require owner authority or SECURITY_MANAGE."""
...
class SecurityGroup(app_commands.Group):
    def __init__(self) -> None:
        super().__init__(name="security", description="Inspect and manage APEXOR security")

    @app_commands.command(name="status", description="Show APEXOR protection status for this server")
    async def status(self, interaction: discord.Interaction) -> None:
        ...
        if guild is None:
            ...
            return
        embed = discord.Embed(title="APEXOR Security", color=discord.Color.blurple())
    ...
    @app_commands.command(name="grant", description="Grant an APEXOR capability to a member")
    ...
    @app_commands.command(name="revoke", description="Revoke an APEXOR capability from a member")
    ...
    @app_commands.describe(enabled="Whether APEXOR should automatically enforce its permission policy")
    async def permission_enforcement(self, interaction: discord.Interaction, enabled: bool) -> None:
        ...
        async with SessionLocal() as session:
            config = await session.scalar(select(SecurityConfig).where(SecurityConfig.guild_id == interaction.guild.id))
            if config is None:
                await interaction.response.send_message("APEXOR security configuration is not initialized yet.", ephemeral=True)
                return
            config.permission_enforcement_enabled = enabled
            await session.commit()
        state = "enabled" if enabled else "disabled"
        await interaction.response.send_message(f"Permission enforcement **{state}**. " + ("APEXOR will now remove critical permissions from manageable non-owner roles." if enabled else "Existing permissions will be audited but not automatically changed."), ephemeral=True)


class ChannelGroup(app_commands.Group):
    def __init__(self) -> None:
        super().__init__(name="channel", description="APEXOR-controlled channel operations")

    @app_commands.command(name="create", description="Create a text channel through APEXOR authorization")
    ...
    async def create(self, interaction: discord.Interaction, name: str, category: discord.CategoryChannel | None = None) -> None:
        if not await _authorized(interaction, Capability.CHANNEL_CREATE):
            return
        assert interaction.guild is not None
        channel = await interaction.guild.create_text_channel(name, category=category, reason=f"APEXOR authorized by {interaction.user.id}")
        await interaction.response.send_message(f"Created {channel.mention}.", ephemeral=True)

    @app_commands.command(name="edit", description="Edit a text channel through APEXOR authorization")
    ...
    async def delete(self, interaction: discord.Interaction, channel: discord.TextChannel, confirm: bool) -> None:
        ...
        if not await _protected_resource_allowed(interaction, resource_type="CHANNEL", resource_id=channel.id):
            await interaction.response.send_message("Protected APEXOR security channels require `SECURITY_MANAGE` or owner authority.", ephemeral=True)
            return
        channel_name = channel.name
        await channel.delete(reason=f"APEXOR authorized by {interaction.user.id}")
        await interaction.response.send_message(f"Deleted `#{channel_name}`.", ephemeral=True)


class RoleGroup(app_commands.Group):
    def __init__(self) -> None:
        super().__init__(name="role", description="APEXOR-controlled role operations")

    @app_commands.command(name="create", description="Create a role through APEXOR authorization")
    ...
    async def create(self, interaction: discord.Interaction, name: str) -> None:
        if not await _authorized(interaction, Capability.ROLE_CREATE):
            return
        assert interaction.guild is not None
        role = await interaction.guild.create_role(name=name, reason=f"APEXOR authorized by {interaction.user.id}")
        await interaction.response.send_message(f"Created role `{role.name}`.", ephemeral=True)

    @app_commands.command(name="edit", description="Edit a role's non-permission metadata through APEXOR authorization")
    ...
    async def edit(self, interaction: discord.Interaction, role: discord.Role, name: str | None = None, hoist: bool | None = None, mentionable: bool | None = None) -> None:
        ...
        if not await _protected_resource_allowed(interaction, resource_type="ROLE", resource_id=role.id):
            await interaction.response.send_message("Protected APEXOR roles require `SECURITY_MANAGE` or owner authority.", ephemeral=True)
            return
    ...
    async def delete(self, interaction: discord.Interaction, role: discord.Role, confirm: bool) -> None:
        ...
        if not await _protected_resource_allowed(interaction, resource_type="ROLE", resource_id=role.id):
            ...
            return
        role_name = role.name
        await role.delete(reason=f"APEXOR authorized by {interaction.user.id}")
        await interaction.response.send_message(f"Deleted role `{role_name}`.", ephemeral=True)


class ModerationGroup(app_commands.Group):
    def __init__(self) -> None:
        super().__init__(name="moderation", description="APEXOR-authorized moderation operations")

    @app_commands.command(name="kick", description="Kick a member through APEXOR authorization")
    ...
    @app_commands.command(name="ban", description="Ban a member through APEXOR authorization")
    ...
    @app_commands.command(name="timeout", description="Timeout a member through APEXOR authorization")
...
class AIGroup(app_commands.Group):
    ...
    def __init__(self) -> None:
        super().__init__(name="ai", description="Inspect APEXOR advisory AI security analysis")
        self._analyst = ThreatAnalyst()
        self._conversation = ConversationalSecurityAnalyst()

    @app_commands.command(name="status", description="Show APEXOR AI availability and latest analysis")
    async def status(self, interaction: discord.Interaction) -> None:
        if not await _authorized(interaction, Capability.SECURITY_VIEW):
            return
        if interaction.guild is None or SessionLocal is None:
        ...
        embed = discord.Embed(title="APEXOR AI Security", color=discord.Color.blurple())
    ...
    async def incident(self, interaction: discord.Interaction) -> None:
        ...
        if incident is None:
            ...
            return
        embed = discord.Embed(title=f"APEXOR Incident {incident.incident_key}", color=discord.Color.red() if incident.severity in {"CRITICAL", "EMERGENCY"} else discord.Color.orange())
    ...
    @app_commands.command(name="ask", description="Ask the advisory APEXOR security analyst a question")
    ...
    async def ask(self, interaction: discord.Interaction, question: app_commands.Range[str, 1, 1000]) -> None:
        ...
        if not self._conversation.enabled:
            await interaction.response.send_message("APEXOR AI is currently unavailable. Configure GROQ_API_KEY and GROQ_MODEL.", ephemeral=True)
            return
        ...
        if guild is None:
            await interaction.followup.send("APEXOR has not initialized this server yet.", ephemeral=True)
            return
        ...
        try:
            ...
            await interaction.followup.send("APEXOR AI could not complete the request. Deterministic security controls are unaffected.", ephemeral=True)
            return

        embed = discord.Embed(title="APEXOR AI Analyst", description=answer, color=discord.Color.blurple())
        embed.set_footer(text="Advisory only • APEXOR deterministic security policy remains authoritative")
        await interaction.followup.send(embed=embed, ephemeral=True)


class APEXORCommandTree(app_commands.CommandTree):
